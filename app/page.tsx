...
import {
  ...
  deal as dealPhaseGame, findPhaseSelection, phaseMeldSizes, reduceGame, scoreCards,
} from "../lib/phase10";
...
type GolfState = { game: "golf"; players: GolfPlayer[]; draw: GolfCard[]; discard: GolfCard[]; turn: number; maxPlayers: number; started: boolean; revealed?: boolean; finalDrawPending?: boolean; winnerId?: string; drawnFromDiscardId?: string };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!player) return;
    const maxSelection = player.laidPhase ? 1 : player.hand.length;
  ...
  const drawGolf = (from: "draw" | "discard") => {
    ...
    setState({ ...state, [from]: pile.slice(0, -1), finalDrawPending: from === "draw" && pile.length === 1, drawnFromDiscardId: from === "discard" ? card.id : undefined });
    setHeldGolf(card); setHeldGolfFromDiscard(from === "discard"); setNotice("Swap it into a position or discard it.");
  };
  const actGolf = (cardIndex?: number) => {
    if (state.game !== "golf" || !heldGolf || state.turn !== localPlayer) return;
    if (cardIndex === undefined) {
      if (heldGolfFromDiscard) { setNotice("You cannot immediately re-discard the card picked up from the discard pile."); return; }
      const nextTurn = (state.turn + 1) % state.players.length;
      const completed = state.finalDrawPending;
      const players = completed ? state.players.map((player) => ({ ...player, score: player.cards.reduce((sum, card) => sum + golfValue(card), 0) })) : state.players;
      const winnerId = completed ? players.reduce((winner, player) => player.score < winner.score ? player : winner, players[0]).id : undefined;
      setState({ ...state, players, discard: [...state.discard, heldGolf], turn: nextTurn, revealed: completed, finalDrawPending: false, winnerId, drawnFromDiscardId: undefined });
    ...
    const completed = state.finalDrawPending;
    const scoredPlayers = completed ? players.map((item) => ({ ...item, score: item.cards.reduce((sum, card) => sum + golfValue(card), 0) })) : players;
    const winnerId = completed ? scoredPlayers.reduce((winner, item) => item.score < winner.score ? item : winner, scoredPlayers[0]).id : undefined;
    setState({ ...state, players: scoredPlayers, discard: [...state.discard, old], turn: (state.turn + 1) % state.players.length, revealed: completed, finalDrawPending: false, winnerId, drawnFromDiscardId: undefined });
