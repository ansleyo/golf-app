...
type GolfState = { game: "golf"; players: GolfPlayer[]; draw: GolfCard[]; discard: GolfCard[]; turn: number; maxPlayers: number; started: boolean; revealed?: boolean; finalDrawPending?: boolean; winnerId?: string };
...
export default function Home() {
  ...
  const drawGolf = (from: "draw" | "discard") => {
    ...
    setState({ ...state, [from]: pile.slice(0, -1), finalDrawPending: from === "draw" && pile.length === 1 });
  ...
  const actGolf = (cardIndex?: number) => {
    ...
    if (cardIndex === undefined) {
      ...
      setState({ ...state, players, discard: [...state.discard, heldGolf], turn: nextTurn, revealed: completed, finalDrawPending: false, winnerId });
    ...
    setState({ ...state, players: scoredPlayers, discard: [...state.discard, old], turn: (state.turn + 1) % state.players.length, revealed: completed, finalDrawPending: false, winnerId });
