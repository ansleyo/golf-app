...
The app stores each room's game discriminator and authoritative state in Supabase, then broadcasts updates through Supabase Realtime. Room codes use `GOLF-XXXX` or `PHASE-XXXX`; the existing open policies are intended for friends-only play and should be tightened before making the app public.
